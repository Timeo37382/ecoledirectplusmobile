...
export default function App({ edpFetch }) {
    ...
    useEffect(() => {
        const handleMessage = (event) => {
            if (event.data.type === "EDP_UNBLOCK") {
                ...
                // Une coquille native ou l'extension est active : on bascule en
                // direct pour que les requetes sortent par l'IP de l'utilisateur.
    ...
    async function setupGtkToken() {
        // En mode proxy, le GTK est obtenu et renvoye cote serveur par le worker
        // lors de la requete de login : il n'y a rien a negocier depuis le navigateur.
        if (isProxyTransport()) {
            return;
        }
        await new Promise((resolve, reject) => {
            ...
            fetch(`${apiRoot()}/v3/login.awp?gtk=1&v=${apiVersion}`)
                .then(() => {
                    setTimeout(() => {
                        window.removeEventListener("message", handleMessage);
    ...
    async function fetchLogin(username, password, keepLoggedIn, callback, controller = (new AbortController())) {
        ...
        try {
            const nresponse = await fetch(`${apiRoot()}/v3/login.awp?v=${apiVersion}`, {
            ...
            if (!text) {
                // En mode direct, une reponse vide signale quasi toujours une
                // extension absente ou inactive. En mode proxy, c'est une vraie
                // erreur reseau : on la remonte au lieu de blamer l'extension.
                if (isProxyTransport()) {
                    messages.submitButtonText = "Echec de la connexion";
                    messages.submitErrorMessage = "Le serveur n'a renvoye aucune reponse. Reessayez dans un instant.";
                    callback(messages);
                    return;
                }
                setIsEDPUnblockInstalled(false);
    ...
    async function fetchUserTimeline(controller = (new AbortController())) {
        ...
        edpFetch(`${apiRoot()}/v3/eleves/${accountsListState[activeAccount].id}/timeline.awp?verbe=get&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
    ...
    async function fetchTimetable({ startDate, endDate, controller = (new AbortController()) }) {
        ...
        try {
            ...
            const timetableResponse = await edpFetch(
                `${apiRoot()}/v3/E/${accountsListState[activeAccount].id}/emploidutemps.awp?verbe=get&v=${apiVersion}`,
                {
                    method: "POST",
                    headers: {
                        "X-Token": tokenState,
            ...
            try {
                const homeworkResponse = await edpFetch(
                    `${apiRoot()}/v3/Eleves/${accountsListState[activeAccount].id}/cahierdetexte.awp?verbe=get&v=${apiVersion}`,
                    {
                        method: "POST",
                        headers: {
    ...
    async function fetchTimetableCoursework({ course, controller = (new AbortController()) }) {
        ...
        try {
            ...
            const response = await edpFetch(
                `${apiRoot()}/v3/Eleves/${accountsListState[activeAccount].id}/cahierdetexte/${courseDate}.awp?verbe=get&v=${apiVersion}`,
                {
                    method: "POST",
                    headers: {
                        "X-Token": tokenState,
    ...
    async function fetchUserGrades(controller = (new AbortController())) {
        ...
        edpFetch(
            `${apiRoot()}/v3/eleves/${accountsListState[userId].id}/notes.awp?verbe=get&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
                    "X-Token": tokenState,
    ...
    async function fetchHomeworks(controller = (new AbortController()), date = "incoming") {
        ...
        if (accountsListState[activeAccount].firstName === "Guest") {
            ...
            edpFetch(
                `${apiRoot()}/v3/Eleves/${accountsListState[userId].id}/${endpoint}.awp?verbe=get&v=${apiVersion}`,
                {
                    method: "POST",
                    headers: {
                        "X-Token": tokenState,
    ...
    async function fetchHomeworksDone({ tasksDone = [], tasksNotDone = [] }, controller = (new AbortController())) {
        ...
        return edpFetch(
            `${apiRoot()}/v3/Eleves/${accountsListState[userId].id}/cahierdetexte.awp?verbe=put&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
                    "X-Token": tokenState,
    ...
    async function fetchMessages(folderId = 0, controller = (new AbortController())) {
        ...
        edpFetch(
            `${apiRoot()}/v3/${accountsListState[userId].accountType === "E" ? "eleves/" + accountsListState[userId].id : "familles/" + accountsListState[userId].familyId}/messages.awp?force=false&typeRecuperation=${specialFolderType}&idClasseur=${folderId}&orderBy=date&order=desc&query=&onlyRead=&getAll=1&verbe=get&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
                    "X-Token": tokenState,
    ...
    async function fetchMessageContent(id, controller) {
        ...
        edpFetch(
            `${apiRoot()}/v3/${accountsListState[userId].accountType === "E" ? "eleves/" + accountsListState[userId].id : "familles/" + accountsListState[userId].familyId}/messages/${id}.awp?verbe=get&mode=${mode}&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
                    "X-Token": tokenState,
    ...
    async function fetchMessageMarkAsUnread(ids = [], controller) {
        ...
        edpFetch(
            `${apiRoot()}/v3/${accountsListState[userId].accountType === "E" ? "eleves/" + accountsListState[userId].id : "familles/" + accountsListState[userId].familyId}/messages.awp?verbe=put&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
                    "X-Token": tokenState,
    ...
    async function fetchSchoolLife(controller = (new AbortController())) {
        ...
        edpFetch(`${apiRoot()}/v3/eleves/${accountsListState[activeAccount].id}/viescolaire.awp?verbe=get&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
                    // "user-agent": navigator.userAgent,
    ...
    function fetchA2F({ method = "get", choice = "", callback = (() => { }), errorCallback = (() => { }), controller = (new AbortController()) }) {
        abortControllers.current.push(controller);
        fetch(
            `${apiRoot()}/v3/connexion/doubleauth.awp?verbe=${method}&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
                    "X-Token": tokenState,
    ...
    async function createFolderStorage(name) {
        ...
        edpFetch(`${apiRoot()}/v3/messagerie/classeurs.awp?verbe=post%26v=4.52.0`,
            {
                method: "POST",
                headers: {
                    "X-Token": tokenState,
    ...
    async function fetchFile(id, type, specialParams) {
        ...
        return await edpFetch(
            `${apiRoot()}/v3/telechargement.awp?verbe=get&fichierId=${id}&leTypeDeFichier=${type}${idDevoir ? `&idDevoir=${idDevoir}` : ""}`,
            {
                method: "POST",
                headers: {
                    "x-token": tokenState
    ...
    async function fetchAdministrativeDocuments(selectedYear, controller = (new AbortController())) {
        abortControllers.current.push(controller);
        return edpFetch(`${apiRoot()}/v3/${accountsListState[activeAccount].accountType === "E" ? "eleves" : "famille"}Documents.awp?archive=${selectedYear}&verbe=get&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
                    "X-Token": tokenState,
    ...
    async function renameFolder(id, name, controller = (new AbortController())) {
        abortControllers.current.push(controller);
        return edpFetch(
            `${apiRoot()}/v3/messagerie/classeur/${id}.awp?verbe=put&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
                    "x-token": tokenState
    ...
    async function deleteFolder(id, controller = new AbortController()) {
        abortControllers.current.push(controller);
        return edpFetch(
            `${apiRoot()}/v3/messagerie/classeur/${id}.awp?verbe=delete&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
                    "x-token": tokenState
    ...
    async function createFolder(name, controller = new AbortController()) {
        abortControllers.current.push(controller);
        return edpFetch(
            `${apiRoot()}/v3/messagerie/classeurs.awp?verbe=post&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
                    "x-token": tokenState
    ...
    async function archiveMessage(id, controller = new AbortController()) {
        abortControllers.current.push(controller);
        return edpFetch(
            `${apiRoot()}/v3/${accountsListState[activeAccount].accountType === "E" ? "eleves/" + accountsListState[activeAccount].id : "familles/" + accountsListState[activeAccount].familyId}/messages.awp?verbe=put&v=${apiVersion}`,
            {
                method: "POST",
                headers: {
                    "x-token": tokenState
